...
import           Pos.Chain.Txp (TxId, TxIn (..), TxOut (..), TxOutAux (..),
                     Utxo)
import           Pos.Core (Address (..), CoinPair, SlotId)
...
prefilterTx wKey tx = ((prefInps',prefOuts'),metas)
    where
        ...
        (prefInCoins  :: (Map HdAccountId CoinPair)) = map (sumCoinPairsUnsafe . map snd . Set.toList) prefInps
        (prefOutCoins :: (Map HdAccountId CoinPair)) = map (\mp -> sumCoinPairsUnsafe $ map (toCoinPair . fst) mp) prefOuts'
...
prefilterInputs :: WalletKey
                -> [(TxIn, ResolvedInput)]
                -> (Bool, Map HdAccountId (Set (TxIn, CoinPair)))
prefilterInputs wKey inps
    ...
    where
        ...
        f ((txIn, out),addrId) = (addrId ^. hdAddressIdParent,
                                     Set.singleton (txIn, toCoinPair out))
...
extendWithSummary (onlyOurInps,onlyOurOuts) utxoWithAddrId
    = Map.fromList $ map toAddrSummary (Map.toList utxoWithAddrId)
    where
        toAddrSummary (txIn@(TxInUtxo txId _), (txOutAux, addressId))
            = (txIn, (txOutAux, addrSummary txId))
          where

...
updateUtxo :: PrefilteredBlock -> (Utxo, Core.CoinPair) -> (Utxo, Core.CoinPair)
updateUtxo PrefilteredBlock{..} (utxo, balance) =
  ...
  where
    ...
    balance'  = balance 
                `Core.unsafeAddCoinPair` 
                Core.unsafeUtxoBalance pfbOutputs 
                `Core.unsafeSubCoinPair` 
                Core.unsafeUtxoBalance utxoMin

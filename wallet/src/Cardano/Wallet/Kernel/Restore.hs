...
import           Cardano.Wallet.Kernel.Util.Core (unsafeUtxoBalance)
...
import           Pos.Chain.Txp (TxIn (..), TxOut (..), TxOutAux (..), Utxo,
                     genesisUtxo)
import           Pos.Core as Core (Address, BlockCount (..), CoinPair, SlotId,
                     flattenSlotId, getCurrentTimestamp, zeroCoinPair)
...
restoreWallet :: Kernel.PassiveWallet
              ...
              -> HD.AssuranceLevel
              -> EncryptedSecretKey
              -> IO (Either CreateHdRootError (HD.HdRoot, CoinPair))
...
getWalletInitInfo coreConfig wKey@(wId, wdc) lock = do
  ...
  where
    ...
    isOurs (inp, out@(TxOutAux txOut)) = do

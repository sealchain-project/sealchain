...
data TxMeta = TxMeta {
    ...
    , _txMetaAmount       :: Core.Coin
      ...
      -- | Total amount
    , _txMetaGDAmount     :: Core.GoldDollar

      -- | Transaction inputs
    , _txMetaInputs       :: NonEmpty (Txp.TxId, Word32, Core.Address, Core.CoinPair)
   
      -- | Transaction    utputs
    , _txMetaOutputs      :: [(Core.Address, Core.CoinPair)]
   
      -- | Transaction    reation time
...
exactlyEqualTo t1 t2 =
    and [ t1 ^. txMetaId == t2 ^. txMetaId
        ...
        , t1 ^. txMetaGDAmount == t2 ^. txMetaGDAmount
        ...
        , t1 ^. txMetaIsGDOutgoing == t2 ^. txMetaIsGDOutgoing
...
instance Buildable TxMeta where
    build txMeta = bprint (" id = "%shortHashF%
                           ...
                           " creationAt = " % F.build %
                           " isOutgoing = " % F.build %
                           " isGDOutgoing = " % F.build %
                           " walletId = " % F.build %
                           " accountIx = " % F.build
                          ) (txMeta ^. txMetaId)
                            (txMeta ^. txMetaAmount)
                            (txMeta ^. txMetaGDAmount)
                            (txMeta ^. txMetaInputs)
                            (txMeta ^. txMetaOutputs)
                            (txMeta ^. txMetaCreationAt)
                            (txMeta ^. txMetaIsOutgoing)
                            (txMeta ^. txMetaIsGDOutgoing)

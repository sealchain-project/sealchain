...
data TxMetaT f = TxMeta {
    ...
    , _txMetaTableGDAmount     :: Beam.Columnar f Core.GoldDollar
    , _txMetaTableCreatedAt    :: Beam.Columnar f Core.Timestamp
    , _txMetaTableIsOutgoing   :: Beam.Columnar f Bool
    , _txMetaTableIsGDOutgoing :: Beam.Columnar f Bool
...
mkTxMeta txMeta = TxMeta {
                ...
                , _txMetaTableGDAmount     = txMeta ^. Kernel.txMetaGDAmount
                , _txMetaTableCreatedAt    = txMeta ^. Kernel.txMetaCreationAt
                , _txMetaTableIsOutgoing   = txMeta ^. Kernel.txMetaIsOutgoing
                , _txMetaTableIsGDOutgoing = txMeta ^. Kernel.txMetaIsGDOutgoing
...
data TxInputT f = TxInputT {
    ...
    , _inputTableGD           :: Beam.Columnar f Core.GoldDollar
    } deriving Generic
...
mkInputs Kernel.TxMeta{..}  = toTxInput <$> _txMetaInputs
    where
        toTxInput :: (Txp.TxId, Word32, Core.Address, Core.CoinPair) -> TxInput
        toTxInput (fTxId, fIndex, addr, (coin, gd)) =
            TxInputT {
...
fromInputs :: NonEmpty TxInput -> NonEmpty (Txp.TxId, Word32, Core.Address, Core.CoinPair)
fromInputs ls = go <$> ls
    where
        go TxInputT{..} = (_inputTableForeignTxid, _inputTableForeignIndex,
                           _inputTableAddress, (_inputTableCoin, _inputTableGD))
...
data TxOutputT f = TxOutputT {
        ...
        , _outputTableGD      :: Beam.Columnar f Core.GoldDollar
    } deriving Generic
...
forOrder :: TxOutputT f0
         -> (Beam.Columnar f0 Txp.TxId, Beam.Columnar f0 Word32,
             Beam.Columnar f0 Core.Address, Beam.Columnar f0 Core.Coin, Beam.Columnar f0 Core.GoldDollar)
forOrder TxOutputT{..} = (_outputTableTxId, _outputTableIndex, _outputTableAddress, _outputTableCoin, _outputTableGD)
...
mkOutputs Kernel.TxMeta{..} = toTxOutput <$> zip _txMetaOutputs [0..]
    where
        toTxOutput :: ((Core.Address, Core.CoinPair), Word32) -> TxOutput
        toTxOutput ((addr, (coin, gd)), index) =
            TxOutputT {
...
fromOutputs :: [TxOutput] -> [(Core.Address, Core.CoinPair)]
fromOutputs ls = go <$> sort ls
  where
    go TxOutputT {..} = (_outputTableAddress, (_outputTableCoin, _outputTableGD))
...
instance HasSqlValueSyntax SqliteValueSyntax Core.GoldDollar where
    sqlValueSyntax = sqlValueSyntax . Core.unsafeGetGoldDollar
...
-- | 'DataType' declaration to convince @Beam@ treating 'Core.GoldDollar(s) as
-- SQLite BIG INTEGER.
...
initialMigration () = do
    MetaDB <$> createTable "tx_metas"
                 (TxMeta (field "meta_id" txIdDT notNull)
                         ...
                         (field "meta_gd_amount" goldDollarDT notNull)
                         (field "meta_created_at" timestampDT notNull)
                         (field "meta_is_outgoing" boolean notNull)
                         (field "meta_is_gd_outgoing" boolean notNull)
           ...
           <*> createTable "tx_metas_inputs"
                 (TxInputT (field "meta_id" txIdDT notNull)
                           ...
                           (field "input_gd" goldDollarDT notNull)
                          )
           <*> createTable "tx_metas_outputs"
                 (TxOutputT (field "meta_id" txIdDT notNull)
                            ...
                            (field "output_gd" goldDollarDT notNull)
                           )
...
toTxMeta TxMeta{..} inputs outputs = Kernel.TxMeta {
    ...
    , _txMetaIsGDOutgoing = _txMetaTableIsGDOutgoing
    ...
    res <- Sqlite.runDBAction $ runBeamSqlite conn $ do
        ...
        case metas of
            [txMeta] -> do
                ...
                outputs <- Just <$> SQL.runSelectReturningList getOutputs
...
getTxMetas conn (Offset offset) (Limit limit) accountFops mbAddress fopTxId fopTimestamp mbSorting = do
    ...
    case res of
        ...
        Right (Just (meta, inputs, outputs)) ->  do
            ...
            let mapWithInputs  = M.map NonEmpty.fromList . transform $ 
                                 map (\inp -> (_inputTableTxId inp, inp)) $ 
                                 NonEmpty.toList inputs

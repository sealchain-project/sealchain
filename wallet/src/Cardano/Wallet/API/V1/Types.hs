...
instance ToJSON (V1 Core.GoldDollar) where
    toJSON (V1 c) = toJSON . Core.unsafeGetGoldDollar $ c

instance FromJSON (V1 Core.GoldDollar) where
    parseJSON v = do
        i <- Core.GoldDollar <$> parseJSON v
        either (fail . toString) (const (pure (V1 i)))
            $ Core.checkGoldDollar i

instance Arbitrary (V1 Core.GoldDollar) where
    arbitrary = fmap V1 arbitrary

instance ToSchema (V1 Core.GoldDollar) where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "V1GoldDollar") $ mempty
            & type_ .~ SwaggerNumber
            & maximum_ .~ Just (fromIntegral Core.maxGoldDollarVal)

instance ToJSON (V1 Core.CoinPair) where
    toJSON (V1 (coins, gds)) = 
        object [ "coins" .= Core.coinToInteger coins
               , "gds"   .= Core.goldDollarToInteger gds ]

instance FromJSON (V1 Core.CoinPair) where
    parseJSON = withObject "CoinPair" $ \o -> do
            coins <- o .: "coins"
            gds <- o .: "gds"
            let coinPair = (Core.Coin coins, Core.GoldDollar gds)
            either (fail . toString) (const (pure (V1 coinPair)))
                $ Core.checkCoinPair coinPair

instance Arbitrary (V1 Core.CoinPair) where
    arbitrary = fmap V1 arbitrary

instance ToSchema (V1 Core.CoinPair) where
    declareNamedSchema _ =
        pure $ NamedSchema (Just "V1CoinPair") $ mempty
            & type_ .~ SwaggerObject
            & required .~ ["coins", "gds"]
            & properties .~ (mempty
...
instance Example (V1 Core.GoldDollar)
instance Example (V1 Core.CoinPair)

...
import           Pos.Core (Address, CoinPair, SlotId, 
                     unsafeAddCoinPair, zeroCoinPair)
...
--
-- NOTE: Does not check that the root exists.
rootTotalBalance :: HdRootId -> Query' e HdWallets CoinPair
rootTotalBalance rootId = do
  ...
  where
    sumTotals :: [CoinPair] -> CoinPair
    sumTotals = foldl' unsafeAddCoinPair zeroCoinPair
...
currentAvailableBalance :: HdAccountId -> Query' UnknownHdAccount HdWallets CoinPair
...
currentTotalBalance :: HdAccountId -> Query' UnknownHdAccount HdWallets CoinPair
...
currentTotalBalance' :: HdAccount -> Query' e HdWallets CoinPair

...
putCurrentUtxoSum = do
  ...
  where
    computeUtxoSum :: (MonadDBRead m, MonadUnliftIO m) => m (Integer, Integer) 
    computeUtxoSum = do
        let valueSource =
                mapOutput (utxoValue . toaOut . snd) utxoSource
        runConduitRes $ valueSource .| CL.fold addValue (0, 0)

    utxoValue (TxOut _ coin) = (coinToInteger coin, 0) 
    utxoValue (TxOutGD _ gd) = (0, goldDollarToInteger gd) 
    utxoValue _ = (0, 0) 

    addValue (c1, gd1) (c2, gd2) = (c1 + c2, gd1 + gd2)
...
sanityCheckBalances = do
    let toPair (TxOut addr coin) = (addr, (coin, mkGoldDollar 0))
        toPair (TxOutGD addr gd) = (addr, (mkCoin 0, gd))
        toPair (TxOutState addr _ _) = (addr, (mkCoin 0, mkGoldDollar 0))
    let utxoBalancesSource =
            mapOutput (toPair . toaOut . snd) utxoSource

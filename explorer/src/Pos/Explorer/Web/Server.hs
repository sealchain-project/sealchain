...
getAddressSummary nm genesisHash cAddr = do
    ...
    balance <- fromMaybe (minBound, minBound) <$> getAddrBalance addr
...
getTxSummary genesisHash cTxId = do
    getTxSummaryFromBlockchain cTxId
  where
    ...
    getTxSummaryFromBlockchain cTxId' = do
        ...
        let totalInput          = unsafeIntegerToCoin . sumCoins $ map snd inputOutputs
        let totalOutput         = unsafeIntegerToCoin . sumCoins $ map snd txOutputs

        let inputGDOutputs      = filterAndConvertTxGDOutputs rawInputsOutputs
        let txGDOutputs         = filterAndConvertTxGDOutputs $ _txOutputs tx
        let totalGD             = unsafeIntegerToGoldDollar . sumGoldDollars $ map snd txGDOutputs
...
getGenesisSummary = do
  ...
  where
    ...
    getRedeemAddressInfo address initialBalance = do
        currentBalance <- fst . fromMaybe (minBound, minBound) <$> getAddrBalance address

...
# USAGE
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   scripts/build/sealchain.sh                 build all packages one-by-one
#   scripts/build/sealchain.sh -t              build and run tests
#   scripts/build/sealchain.sh core|db|...     build only a specific project (see below)
#   scripts/build/sealchain.sh -k              typecheck but do not build
#   scripts/build/sealchain.sh -c              do stack clean
#
# Consider symlinking the script as `b` into the sealchain folder because
# typing `scripts/build/sealchain.sh` is annoying.

# PROJECTS
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#   Project argument                Package name
#   :
#   core, db, etc.                  sealchain-{core,db,etc.}
#   lib                             sealchain
...
projects="auxx binary chain client core crypto db explorer generator infra networking node tools tools-post-mortem util node-ipc x509 mnemonic cluster wallet"
...
do
  ...
  # benchmarks config
    ...
    # We want:
    # • --flag sealchain-core:-asserts ($asserts)
...
commonargs='--no-haddock-deps --jobs=4'
...
if [[ "$for_installer" == true ]]; then
  commonargs="$commonargs --flag sealchain-tools:for-installer"
fi

if [[ "$asserts" == false ]]; then
  commonargs="$commonargs --flag sealchain-core:-asserts"
fi
...
if [[ "$spec_prj" == "" ]]; then
  ...
  to_build="sealchain-node sealchain-wallet"
elif [[ "$spec_prj" == "explorer" ]]; then
  to_build="sealchain-node sealchain-explorer"

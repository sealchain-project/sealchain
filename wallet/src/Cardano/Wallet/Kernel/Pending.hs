...
-- @CoinPair@ the sum of the coin of our outputs.
type PartialTxMeta = CoinPair -> TxMeta
...
newForeign w accountId tx meta = do
    map void <$> newTx w accountId tx (\_ ->  meta) $ \ourAddrs ->
    ...
    case res of
        ...
        Right () -> do
            ...
            let myCredentials = filter (\(WalletIdHdRnd hdRoot, _) -> accountId ^. hdAccountIdParent == hdRoot) allCredentials
                ...
                gainedOutputCoins = sumCoinPairsUnsafe ourOutputCoins
                txMeta = partialMeta gainedOutputCoins
            putTxMeta (walletPassive ^. walletMeta) txMeta
            submitTx
            return (Right txMeta)
    where
        (txOut :: [TxOut]) = (_txOutputs . taTx $ tx)
        ...
        allOurs :: [(WalletId, EncryptedSecretKey)] -> [(HdAddress, CoinPair)]
        allOurs = concatMap ourAddrs

        ourAddrs :: (WalletId, EncryptedSecretKey) -> [(HdAddress, CoinPair)]
        ourAddrs (wid, esk) =
            map f $ filterOurs wKey txOutAddress txOut
            where
                nm = makeNetworkMagic $ walletPassive ^. walletProtocolMagic
                f (txOut',addressId) = (initHdAddress addressId (txOutAddress txOut'), toCoinPair' txOut')

...
module Pos.Explorer.Web.ClientTypes
       ...
       , toCTxId
       , fromCTxId
       , filterAndConvertTxOutputs
       , filterAndConvertTxGDOutputs
       , toBlockEntry
       , toTxEntry
       , toBlockSummary
       , toTxBrief
...
import           Pos.Chain.Txp (Tx (..), TxId, TxOut (..), TxOutAux (..),
                     TxUndo, txpTxs, _txOutputs, isOriginTxOut, isGDTxOut)
import           Pos.Core (Address, Coin, EpochIndex, LocalSlotIndex, SlotCount,
                     SlotId (..), StakeholderId, Timestamp, GoldDollar, 
                     addressF, decodeTextAddress, getEpochIndex,
                     getSlotIndex, mkCoin, sumCoins, timestampToPosix,
                     unsafeAddCoin, unsafeGetCoin, unsafeIntegerToCoin,
                     unsafeSubCoin, mkGoldDollar, sumGoldDollars, unsafeGetGoldDollar,
                     unsafeIntegerToGoldDollar, unsafeAddGoldDollar)
...
toBlockEntry epochSlots (blk, Undo{..}) = do
    ...
    let cbeEpoch       = getEpochIndex epochIndex
        ...
        addOutCoins c  = unsafeAddCoin c . totalTxOutCoin
        totalRecvCoin  = unsafeIntegerToCoin . sumCoins $ map totalTxInCoin undoTx
...
totalTxOutCoin :: Tx -> Coin
totalTxOutCoin =
    unsafeIntegerToCoin . sumCoins . map txOutValue . filter isOriginTxOut . _txOutputs
...
totalTxOutGD =
    unsafeIntegerToGoldDollar . sumGoldDollars . map txOutGD . filter isGDTxOut . _txOutputs
...
toTxEntry ts tx = CTxEntry {..}
  where
    ...
    cteAmount     = mkCCoin $ totalTxOutCoin tx
...
filterAndConvertTxOutputs :: [TxOut] -> [(CAddress, Coin)]
filterAndConvertTxOutputs = 
    map (toCAddress . txOutAddress &&& txOutValue) . filter isOriginTxOut

filterAndConvertTxGDOutputs :: [TxOut] -> [(CAddress, GoldDollar)]
filterAndConvertTxGDOutputs = 
    map (toCAddress . txOutAddress &&& txOutGD) . filter isGDTxOut
...
toTxBrief txi = CTxBrief {..}
  where
    ...
    txGDInputs    = filterAndConvertTxGDOutputs inputOutputs
    txGDOutputs     = filterAndConvertTxGDOutputs $ _txOutputs tx

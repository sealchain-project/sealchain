...
cpAvailableBalance :: IsCheckpoint c => c -> Core.CoinPair
cpAvailableBalance c =
    fromMaybe subCoinErr balance'
  where
    pendingIns   = Pending.txIns $ c ^. cpPending
    spentUtxo    = Core.utxoRestrictToInputs (c ^. cpUtxo) pendingIns
    spentBalance = Core.unsafeUtxoBalance spentUtxo
    balance'     = Core.subCoinPair (c ^. cpUtxoBalance) spentBalance
...
cpTotalBalance :: IsCheckpoint c => IxSet (Indexed HdAddress) -> c -> Core.CoinPair
cpTotalBalance ours c =
    Core.unsafeAddCoinPair availableBalance changeBalance
  where
    availableBalance = cpAvailableBalance c
    changeBalance    = Core.unsafeUtxoBalance (cpChange ours c)
